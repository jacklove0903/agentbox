...
| **游客试用模式** | 未登录可直接使用聊天/翻译/总结/文生图，服务端按功能限额计数并可查询剩余次数 |
| **暗色模式** | 全局 Light / Dark 主题切换 |
| **Markdown 渲染** | 代码高亮 + 一键复制 + GFM 表格 + 思考过程折叠 |
| **对话导出** | 导出为 Markdown 文件 |
| **模型管理** | 已登录用户可在侧边栏新增/删除模型（实时生效并刷新后端模型缓存） |
| **登录状态自恢复** | Token 失效自动清理本地状态并回落游客模式，避免卡死在无效登录态 |
...
| `/api/models/create` | POST | 新增模型（需登录） |
| `/api/models/{id}` | DELETE | 删除模型（需登录） |
| `/api/trial/status` | GET | 游客试用剩余次数 |
...
当前版本为阶段性完结版本，核心目标均已交付：

- 多模型并排实时对话与会话管理
- 多工具面板（翻译 / 网页总结 / 图片生成 / Prompt 增强）
- 游客试用 + 登录态完整闭环
- 模型管理（新增 / 删除）
- 流式上下文处理与会话持久化（PostgreSQL）
